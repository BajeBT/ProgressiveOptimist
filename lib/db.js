...
// Throttle record for automated alerts, so a failing integration cannot email
// the officers on every single request. Routes that raise alerts call this
// themselves - api/gallery.js does not run the applicant migration.
export async function ensureSystemAlerts() {
  await sql`
...
// The Google Photos shared albums shown in the gallery. Previously hardcoded in
// the client and overridden per browser via localStorage, so albums were never
// really shared. api/shared-albums.js calls this itself.
export async function ensureSharedAlbums() {
  await sql`
...
export async function ensureApplicantTables() {
  try {
    await sql`ALTER TABLE members ADD COLUMN IF NOT EXISTS approved_by TEXT;`;
    await sql`ALTER TABLE members ADD COLUMN IF NOT EXISTS approved_at TIMESTAMPTZ;`;
    await sql`ALTER TABLE members ADD COLUMN IF NOT EXISTS member_status VARCHAR(20) DEFAULT 'active';`;
    // Set by the member themselves to keep their number out of the members directory.
    await sql`ALTER TABLE members ADD COLUMN IF NOT EXISTS hide_phone BOOLEAN DEFAULT FALSE;`;
    // Name changes are the one profile field a member cannot set themselves -
    // an exec reviews them here, and never their own (api/members.js enforces
    // that the reviewer is not the requester).
    ...
    console.error('ensureApplicantTables migration error:', err);
  }
}
